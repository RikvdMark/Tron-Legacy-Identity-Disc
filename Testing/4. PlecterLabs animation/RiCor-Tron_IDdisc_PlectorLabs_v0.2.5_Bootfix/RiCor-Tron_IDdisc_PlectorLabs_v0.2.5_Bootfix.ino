...
/*Buttos, switches and sensors */
bool oldState1 = HIGH; /* switch 1 */
bool oldState2 = HIGH; /* switch 1 */
bool oldState3 = HIGH; /* switch 1 */
int showType1 = 0; /* switch 1 */
int showType2 = 2; /* switch 1 */
int showType3 = 4; /* switch 1 */

/**************************/
// ERV PLECTER GLOBAL VARIABLES
...
// RiCor: Set 4th digit to 0 for now to disable white pixel
// Based on Erv Plecter's sketch.
...
/*********************************/
  ...
  //musicPlayer.reset(); /* - Performs a hard reset of the chip */
    //-OR- musicPlayer.softReset(); /* - Attempts a soft reset of the chip */
  //delay(1000); /* Give the Music Maker a second to reset */
...
/*********************************/
  ...
  strip.setBrightness(BRIGHTNESS2); /* Set (limit) brightness of the NeoPixel strip for the inner c-ring */
  ...
  Serial.println("> C-ring activated");         /* Serial message for testing purposes */
  musicPlayer.pausePlaying(true);               /* RiCor: Pause (stop) all sounds/music. */
  musicPlayer.startPlayingFile("track050.wav"); /* Play disc boot sound */
  SelectedSequence = SEQUENCE_BOOT;             /* Select C-RING boot animation */
  InitSequence();                               /* Trigger "InitSequence" function */
...
{ 
/****************************************************/
  /* Code that checks button 1. This button is used to control the blade */
...
/*******************/
  /* Code that checks button 2. This button is used for turning the music on/off */
  ...
  if (newState2 == LOW && oldState2 == HIGH)
  {
    ...
    {
      showType2++;
      if (showType2 > 3) /* for this button "ShowType2" toggles between 2 and 3 */
        showType2=2;
...
/*******************/
  /* Code that checks button 3. This button is used for going to the next song */
...
/****************************************************/
  /* RiCor: Part of Erv Plecters animation code.
   * This code continuously checks the "SelectedSequence" variable and triggers the selected animation
   * in the "Animate" function.
   * Breakdown;
   * - Buttonpress is parsed to the "ParseSyntax" function;
   * - "ParseSyntax" function triggers the “InitSequence” function with the selected animation sequence via 
   *    the "Selected Sequence” unsigned char variable. Can also change disc colour;
   * - Erv's code here in the "void loop" triggers the selected animation in the "Animate" function.
   */
  ...
  if((timeElapsed > CurrentEventPeriod) && !IdleFlag )
  {
      ...
      Animate(); /* RiCor: Trigger the "Animate" function with the selected animation in the
                    "SelectedSequence" variable" */   
      CurrentSequenceStep++;
...
/*********************************/
/* Initializes disc in Sam or Rinzler mode or switchover lateron */
void InitBlade(byte WhichDisc)
{
  byte r, g, b;
  uint16_t i;
...
void ParseSyntax(int i) 
{
  ...
  switch(i)
  {
    case 9: /* not required, triggers upon boot in "void setup" */
      Serial.println("C-Ring activated"); /* RiCor: Serial message for testing purposes */
      musicPlayer.pausePlaying(true);     /* RiCor: Pause (stop) all sounds/music.
                                             The Music Maker can't play two files at once so this command first 
                                             stops the blade sound before starting the blade deactivation sound */
      musicPlayer.startPlayingFile("track050.wav"); /* Play disc boot sound */
    ...
    case 1:
      ...
      musicPlayer.pausePlaying(true);     /* RiCor: Pause (stop) all sounds/music */
      musicPlayer.startPlayingFile("track060.wav"); /* RiCor: Play blade sound 
                                                     * Works but slows down animation until file finishes playing... */
    ...
    case 0:
      ...
      musicPlayer.pausePlaying(true);       /* RiCor: Pause (stop) all sounds/music */
      musicPlayer.startPlayingFile("track061.wav"); /* RiCor:  Play blade deactivation sound */
    ...
    case 'l': //RiCor: this is the letter "L", not the number "1"
...
void InitSequence(void) /* Parses selection from the "ParseSyntax" function to "SelectedSequence" variable. 
                           The Time Management section in the “void loop” then uses that to trigger the 
                           "Animate" function. */
{
  uint16_t i;
