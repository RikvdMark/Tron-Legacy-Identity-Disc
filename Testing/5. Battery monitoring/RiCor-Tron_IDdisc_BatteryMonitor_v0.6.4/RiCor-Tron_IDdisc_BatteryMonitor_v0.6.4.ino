...
#define VBATPIN A7     /* Pin connected to the Voltage divider */
#define LowBattLED A4  /* Pin connected to the 'LOW battery' LED */
#define CritBattLED A3 /* Pin connected to the 'Critical battery' LED */

/*********************************/
float LowVoltage = 3.30;  /* This value sets the desired "LOW" voltage */ 
float CritVoltage = 3.10; /* This value sets the desired "CRITICAL" voltage */ 
...
void setup()
{
  ...
  pinMode(CritBattLED, OUTPUT); /* Configure the pin connected to the "LOW voltage indicator" LED as an output pin */
...
void loop()
{
  ...
  measuredvbat *= 1.97; // Multiply by 3.3V, our reference voltage
  measuredvbat /= 1024; // convert to voltage
  Serial.print("VBat: " ); 
  Serial.println(measuredvbat);
    
  /*********************************/
  //Turn on the LED if the voltage of the battery is LOW
  
  if (measuredvbat >= LowVoltage)  /* If the battery voltage is -greater than- or -equal to- the "LowVoltage" value */
  {
    digitalWrite(LowBattLED, LOW); /* The LED indicating the battery voltage is LOW stays off */
  }
  else /* If the battery voltage is -less than- the "LowVoltage" value */
  {
    digitalWrite(LowBattLED, HIGH); /* Turn on the LED indicating the battery voltage is LOW */
    Serial.println("LOW Battery Voltage!"); /* Serial message */
  }

  /*********************************/
  //Turn on the LED if the voltage of the battery is CRITICAL
  
  if (measuredvbat >= CritVoltage)  /* If the battery voltage is -greater than- or -equal to- the "CritVoltage" value */
  {
    digitalWrite(CritBattLED, LOW); /* The LED indicating the battery voltage is CRITICAL stays off */
  }
  else /* If the battery voltage is -less than- the "CritVoltage" value */
  {
    digitalWrite(CritBattLED, HIGH); /* Turn on the LED indicating the battery voltage is CRITICAL */
